  ...
  aria-labelledby="offcanvasScrollingLabel">
	...
	<div class="container my-5">
							<div class="text-center">
                                <h1 class="h4 text-gray-900 m-3">Assign Evacuation Center</h1>
                            </div>
		<div class="table-responsive">
		<div class="col-12">
	<table class="table table-bordered table-sm border-dark dataTable" id="dataTable" width="100%" cellspacing="0" role="grid" aria-describedby="dataTable_info" style="width: 100%;">
        <thead>
                <tr role="row">
		...
		<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 30px;">
		Village/Compound/<br>Subdivision
		</th>
		<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Age: activate to sort column ascending" style="width: 60px;">
		Street
		</th>
		...
		<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-label="Salary: activate to sort column ascending" style="width: 64px;">
		Contact
		</th>
		<th class="sorting" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="2" aria-label="Salary: activate to sort column ascending" style="width: 30px;">
		Evacuation Center Designation (choose one)
		</th>
        ...
        <tbody>
			...
			<?php
				$evccntr = "SELECT * FROM info WHERE evacID = '1' AND usertype = 'user' OR usertype = '' AND evacID = '1'";
				$resu = $con-> query($evccntr);
		...
		echo "<tr><td><b>". $rw["lastname"]."</b></td><td><b>". $rw["home_no"]."</b></td><td>".$rw["village"]."</td><td>". $rw["street"]."</td><td>". $rw["email"]."</td><td>". $rw["contact"]."</td><td>". "<a href=\"evac1.php?email=" .$rw['email']. "\">Tabing Ilog Elementary School</a>"."</td><td>"."<a href=\"evac2.php?email=" .$rw['email']. "\">St Joseph Basketball Court</a>"."</td></tr>";		
			
				$mem = "SELECT * FROM memberlist WHERE addid = '".$rw["hshID"]."'";
				$res = $con-> query($mem);

				if($res -> num_rows > 0 )
					{
						$cnt=1;
		echo "<tr><td>".$cnt++."</td><td><b>Firstname:\n</b>".  $rw["firstname"]."</td><td><b>Lastname:\n</b>". $rw["lastname"]."</td><td><b>Age:\n</b>".$rw["age"]."</td><td><b>Gender:\n</b>". $rw["gender"]."</td><td><b>Role:\n</b>". $rw["role"]."</td><td><b>Contact:\n</b>". $rw["contact"]."</td><td>Family Member</td></tr>";
						
						while ($row = $res -> fetch_assoc())
							{	

			echo "<tr><td>". $cnt++."</td><td><b>Firstname:\n</b>".  $row["fname"]."</td><td><b>Lastname:\n</b>". $row["lname"]."</td><td><b>Age:\n</b>".$row["ages"]."</td><td><b>Gender:\n</b>". $row["gnder"]."</td><td><b>Role:\n</b>". $row["roles"]."</td><td><b>Contact:\n</b>". $row["cont"]."</td><td>Family Member</td></tr>";		
							}
							$cnt--;
							echo "<tr><td colspan='2'><p style='font-weight: 900;'>total = ". $cnt."</p></td></tr>";
					} else {						
							}
								}
						echo "</tbody>";
...
<!--------------------------------------------------------------------------------------------------------------------------------------->		
		...
		<th class="sorting sorting_asc" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending" style="width: 50px;">
			Maximum capacity of facility:
                                    ...
                                    <tbody>
							<?php
										$ti = "SELECT COUNT(*) FROM evacuation WHERE evac = 'TabingIlog' ";
										$num = $con-> query($ti);
									...
									{
										echo "<tr><td>Tabing Ilog Elementary School</td><td>". $row["COUNT(*)"]."</td><td>70</td></tr>";
										}
									}
										else {
									...
									?>
									<?php
										$stj = "SELECT COUNT(*) FROM evacuation WHERE evac = 'StJoseph' ";
										$num2 = $con-> query($stj);

									if($num2 -> num_rows > 0 )
									{
									while ($row1 = $num2 -> fetch_assoc())
									{
										echo "<tr><td>St Joseph Basketball Court</td><td>". $row1["COUNT(*)"]."</td><td>54</td></tr>";
											}
									}
										else {
...
<!--------------------------------------------------------------------------------------------------------------------------------------->						<div class="table-responsive">
			<div class="col-sm-6">
                ...
                <thead>
          <tr role="row">
			<th class="sorting sorting_asc" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Name: activate to sort column descending" style="width: 50%;">
			Evacuation Center
		</th>
		...
		<th class="sorting sorting_asc" tabindex="0" aria-controls="dataTable" rowspan="1" colspan="2" aria-sort="ascending" aria-label="Name: activate to sort column descending" style="width: 20%;">
            ...
            <tbody>
	<?php
			$list1 = "SELECT * FROM evacuation WHERE evac = 'TabingIlog' AND NOT(family='')";
			$edit = $con-> query($list1);

				if($edit -> num_rows > 0 )
					{
				while ($chg = $edit -> fetch_assoc())
					{
			echo "<tr><td>Tabing Ilog Elementary School</td><td>". $chg["family"]."</td><td><a href=\"changeevac.php?eid=" .$chg['eid']. "\">Change Assignment</a></td><td><a href=\"deleteevac.php?eid=" .$chg['eid']. "\">Delete</a></td></tr>";
										}
									}
										else {
				...
				$list2 = "SELECT * FROM evacuation WHERE evac = 'StJoseph' AND NOT(family='')";
				$edit2 = $con-> query($list2);

				if($edit2 -> num_rows > 0 )
				{
					while ($chg2 = $edit2 -> fetch_assoc())
						{
					echo "<tr><td>St Joseph Basketball Court</td><td>". $chg2["family"]."</td><td><a href=\"changeevac.php?eid=" .$chg2['eid']. "\">Change Assignment</a></td><td><a href=\"deleteevac.php?eid=" .$chg2['eid']. "\">Delete</a></td></tr>";
						}
							}
				else {								
				}			?>
